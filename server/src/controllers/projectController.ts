        ...
        const formattedProjects = projects.map((p) => ({
            ...
            assigned_to: p.talent
                ...
                : p.team
                    ...
                    : p.agency
                        ? { id: p.agency.id, name: p.agency.agency_name, type: 'agency', image_url: p.agency.user.avatar_url }
                        : undefined,
        }));
    ...
    try {
        ...
        const formattedProject = {
            ...
            assigned_to: project.talent
                ? { id: project.talent.id, name: project.talent.user.full_name, type: 'talent', image_url: project.talent.user.avatar_url }
                : project.team
                    ? { id: project.team.id, name: project.team.team_name, type: 'team', image_url: project.team.image_url }
                    : project.agency
                        ? { id: project.agency.id, name: project.agency.agency_name, type: 'agency', image_url: project.agency.user.avatar_url }
                        : undefined,
        };
...
export const createProject = async (req: AuthRequest, res: Response) => {
    ...
    try {
        const { name, description, status, progress, budget_spent, total_budget, next_milestone } = req.body;
