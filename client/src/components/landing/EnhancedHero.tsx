...
export default function EnhancedHero() {
    ...
    useEffect(() => {
        ...
        if (p === '/' || p === '' || p.includes('browse-talent') || p.includes('/talent')) {
            setActiveTab('talent');
        } else if (p.includes('browse-teams') || p.includes('/team')) {
            setActiveTab('team');
        } else if (p.includes('browse-agencies') || p.includes('/agency')) {
            setActiveTab('agency');
        }
    }, [pathname]);

    const goTo = async (tab: { id: string; link: string }) => {
        setActiveTab(tab.id); // optimistic UI
        await router.push(createPageUrl(tab.link));
    };
    ...
    return (
        <section className="relative pt-8 pb-32 overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col sm:flex-row justify-center mb-16 items-center gap-2">
                                      <span className="text-sm font-light opacity-90 text-gray-800">I'm looking for</span>

                    <motion.div
                        initial={{ opacity: 0, y: -20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5 }}
                        className="inline-flex items-center gap-2 text-sm text-black boder-2 p-1.5 rounded-full shadow-sm  border-primary"
                    >
                            ...
                            {[
                                ...
                                <button
                                    ...
                                    className={`px-6 py-2 rounded-full text-sm font-bold transition-all duration-300 ${activeTab === tab.id
                                        ? 'bg-white text-primary border-2 border-primary shadow-xl ring-1 ring-black/5'
                                        : '  hover:text-gray-900 hover:bg-gray-300/50 cursor-pointer'
                                        }`}
                                >
                                    {tab.label}
                                </button>
                        ...
                        <div className="relative z-10">
                            <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-gray-200/50 border border-white/50">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                                    ...
                                    <p className="text-lg text-gray-200 font-medium">Senior Product Designer</p>
                                    <p className="text-sm text-gray-300 mt-1">Previously at Apple & Google</p>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                <p className="text-xs text-gray-500 leading-tight">Projects completed successfully on time</p>
                            </motion.div>
                        </div>
                    </motion.div>
                </div>
