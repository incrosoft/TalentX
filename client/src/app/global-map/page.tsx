...
export default function GlobalMapPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [talentsData, teamsData, agenciesData] = await Promise.all([
                    talentXApi.entities.Talent.list(),
                    talentXApi.entities.Team.list(),
                    talentXApi.entities.Agency.list()
                ]);
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white p-1 rounded-xl shadow-sm border border-gray-200">
                    <GlobalMap talents={talents} teams={teams} agencies={agencies} />
                </div>
            </main>
        </div>
    );
